...
open class SysPermissionServiceImpl : ServiceImpl<SysPermissionMapper, SysPermission>(), ISysPermissionService {
     ...
     * 查询所有菜单权限
     */
    override fun findAllSysPermission(): List<SysPermission> {
        return permissionMapper!!.selectList(Wrappers.emptyWrapper<SysPermission>())
    }

    /**
     * 通过用户查询
     */
    override fun findSysPermissionByLoginUser(sysUser: SysUser): List<SysPermission> {
        return permissionMapper!!.getUserPermissionByRoleId(sysUser.roleId!!)
    }

    /**
     * 查询某用户的 角色  菜单列表   权限列表
     */
    override fun getUserPermission(userId: Long): JSONObject {
        return getUserPermissionFromDB(userId)
    }

    /**
     * 从数据库查询用户权限信息
     */
    private fun getUserPermissionFromDB(userId: Long): JSONObject {
        val userPermission = permissionMapper!!.getUserPermission(userId)
        //管理员角色ID为1
        val adminRoleId = 1
        //如果是管理员
