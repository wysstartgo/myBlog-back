...
import com.ambition.business.exception.ExcelException;
...
public class ExcelUtil {
    ...
    public static <T extends BaseRowModel> List<T> readExcel(MultipartFile excel,Class<T>  rowModel) throws ExcelException {
        return readExcelToModel(excel, rowModel);
    }

    private static <T extends BaseRowModel> List<T> readExcelToModel(MultipartFile excel, Class<T> rowModel) throws ExcelException {
     ...
     */
    public static <T extends BaseRowModel> List<T> readExcel(File excel,Class<T>  rowModel) throws ExcelException, FileNotFoundException {
        ExcelListener excelListener = new ExcelListener();
        ExcelReader reader = getReader(excel, excelListener);
        if (reader == null) {
            return new ArrayList<>();
     ...
     * @throws FileNotFoundException
     */
    public static <T extends BaseRowModel> List<T> readExcel(File file,Class<T> rowModel,int sheetNo,int headLineNum) throws ExcelException, FileNotFoundException {
        ExcelListener excelListener = new ExcelListener();
     ...
     * @throws FileNotFoundException
     */
    private static ExcelReader getReader(File file, ExcelListener excelListener) throws ExcelException, FileNotFoundException {
        if (file == null || !file.exists()){
            throw new ExcelException("文件不存在!");
        }
        return  new ExcelReader(new BufferedInputStream(new FileInputStream(file)),null,excelListener,false);
    }
